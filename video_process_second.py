...
DEFAULT_ACTIVITY = "robbery, stealing, pilferage, pilfering, filching, shoplifting, thievery, purloining, embezzlement, hijacking, larceny, Colloq lifting, appropriation, pocketing, pinching, swiping, snitching, Chiefly Brit nicking, US boosting, Slang heist, knocking off, rip-off"  # Replace with your activity label


device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

processor = AutoProcessor.from_pretrained(MODEL_NAME, device_map = 'cuda')
model = AutoModel.from_pretrained(MODEL_NAME, device_map = 'cuda').to(device)
...
def process_video_chunks(video_chunks, activity):
    ...
    for chunk, start_time, end_time in video_chunks:
        ...
        all_results.append({
            ...
            "video_chunk": chunk,
            "max_prob_activity": probs[0][0].item() * 100,  # Probability of the activity in the first position of activities_list
            "start_time": start_time,
            "end_time": end_time
        })
    return all_results

def model_interface_second(uploaded_video, activity = DEFAULT_ACTIVITY):
    ...
    for result in all_results:
        if result["max_prob_activity"] > THRESHOLD_PROBABILITY:
            small_chunks = split_video_into_chunks(uploaded_video, SMALL_CHUNK_DURATION, fps)
    ...
    if best_result["likely_label"] != "other":
        segment_path = os.path.join(SAVE_DIR, f"{uuid.uuid4()}.mp4")
        ffmpeg_extract_subclip(uploaded_video, best_result["start_time"], best_result["end_time"], targetname=segment_path)
        print(f"{datetime.now()} - The best video segment has been saved as '{output_filename}'")
    else:
        print(f"{datetime.now()} - The most likely activity is 'other'. No segment was saved.")

    return best_result, segment_path
